...
export class SpectralWavesRenderer {
  ...
  public render(
    ...
    waves.forEach((wave, waveIndex) => {
      // Update wave phase
      this.wavePhases[waveIndex] =
        (this.wavePhases[waveIndex] ?? 0) + 0.015 * (1 + waveIndex * 0.1);
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        const modulation =
          Math.sin((i / barCount) * Math.PI * 2 + this.timeOffset * 0.5) * 0.3;
        ...
        const y =
          ...
          wave.offset +
          Math.sin((i / barCount) * Math.PI * 8 + this.timeOffset * 2) *
            normalizedValue *
            8;
